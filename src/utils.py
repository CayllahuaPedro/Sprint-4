...
class ImageDownloader:
    ...
    def download_images(self, df, print_every=1000, workers=16):
        from concurrent.futures import ThreadPoolExecutor, as_completed

        image_paths = [np.nan] * len(df)
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {executor.submit(download_one, (i, row)): i for i, row in enumerate(rows)}
            for future in as_completed(futures):
                pos, path = future.result()
                image_paths[pos] = path
                completed += 1
                if completed % print_every == 0:
                    print(f"Downloaded {completed}/{len(df)}")
